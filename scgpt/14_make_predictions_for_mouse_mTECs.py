...
run = wandb.init(
    ...
    project="Human_sc_mTEC_scGPT_predict_for_mouse_mimetic",
    reinit=True,
...
max_seq_len = 16000 # Set max_seq_len to the number of genes in the sc dataset (or slightly larger)
...
save_dir = Path(f"./save_human_scGPT/dev_{dataset_name}-{time.strftime('%b%d-%H-%M')}_predict_for_mouse_mimetic/")
...
## "batch_id" 1 for query
## Change data_is_raw accordingly

if dataset_name == "mouse_mtec":
    data_dir = Path("/n/groups/cbdm_lab/brh040/analysis/scGPT/mimetics-only_version/mouse")
    
    adata = sc.read(data_dir / "meclo_seurat_table_mouse-humanlabels_mimeticsonly-20240306_cleaned.h5ad")
...
adata_test_cell_emb.write(save_dir / "adata_test_cell_emb_large_v2_mouse_mimetic.h5ad")
...
emb_df.to_csv(save_dir / 'mouse_embeddings_mimetics-only_version.csv', index=True)

adata_emb.obs.to_csv(save_dir / 'mouse_obs_info_mimetics-only_version.csv', index=True)
