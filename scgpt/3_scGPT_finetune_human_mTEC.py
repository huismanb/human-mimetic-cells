...
# -*- coding: utf-8 -*-
"""
Finetune scGPT on human mTEC scRNA-seq data

modified from https://github.com/bowang-lab/scGPT/blob/main/tutorials/Tutorial_Annotation.ipynb
"""
...
# set include_zero_gene = True

hyperparameter_defaults = dict(
    seed=0,
    dataset_name="human_mtec",
    do_train=True,
    load_model="/n/groups/cbdm_lab/brh040/analysis/scGPT/mimetics-only_version/human_whole_checkpoint/scGPT_human",
    mask_ratio=0.0,
    epochs=15,
    n_bins=51,
...
save_dir = Path(f"./save_human_scGPT/dev_{dataset_name}-{time.strftime('%b%d-%H-%M')}_mimetics-only/")
...
if dataset_name == "human_mtec":
    ...
    adata_test = adata[adata.obs["orig.ident"]=="HT6",:].copy() #n_obs × n_vars = 6190 × 31238
    adata_ref = adata[adata.obs["orig.ident"].isin(["HT2", "HT3", "HT4", "HT5"]),:].copy() #n_obs × n_vars = 22014 × 31238
...
# Used raw data: non normed or loged
...
## define train, evaluate and define_wandb_metrcis
## TODO:
# Examine the meaning of total_loss, etc. in the function "evaluate" below
# can change return_raw in "evaluate" to TRUE
...
for epoch in range(1, epochs + 1):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(best_model.state_dict(), save_dir / f"model_epoch_{str(best_model_epoch)}.pt")
...
# save the model into the save_dir
torch.save(best_model.state_dict(), save_dir / "model_best.pt")
