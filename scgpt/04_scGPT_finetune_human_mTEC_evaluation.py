...
hyperparameter_defaults = dict(
    seed=0,
    dataset_name="human_mtec",
    do_train=True,
    #load_model="for_brooke/scGPT/save_human_scGPT/best_102623",
...
run = wandb.init(
    ...
    project="human_mtec_scGPT_mimetics-only",
    reinit=True,
...
max_seq_len = 22000 # Set max_seq_len to the number of genes in the sc dataset (or slightly larger)
...
save_dir = Path(f"./save_human_scGPT/dev_{dataset_name}-{time.strftime('%b%d-%H-%M')}_mimetics-only_evaluation/")
...
if dataset_name == "human_mtec":
    data_dir = Path("/n/groups/cbdm_lab/brh040/analysis/scGPT/mimetics-only_version/human_data")
    adata = sc.read(data_dir / "meclo_ht2-ht6_seurat_table_20240309_labelled-mimeticsonly_preprocessed.h5ad")	

    adata_test = adata[adata.obs["orig.ident"]=="HT6",:].copy() 
    adata_ref = adata[adata.obs["orig.ident"].isin(["HT2", "HT3", "HT4", "HT5"]),:].copy() 
    ...
    adata_ref.obs["batch_id"] = adata_ref.obs["str_batch"] = "0"
    adata_test.obs["batch_id"] = adata_test.obs["str_batch"] = "1"     

    print(np.sum(adata.var.index == adata_ref.var.index))
    print(np.sum(adata.var.index == adata_test.var.index))
    adata_ref.var.set_index(adata.var.index, inplace=True)
    adata_test.var.set_index(adata.var.index, inplace=True)
    print(np.sum(adata_ref.var.index == adata_test.var.index))

    data_is_raw = True
    filter_gene_by_counts = False
    adata_test_raw = adata_test.copy()
    adata = adata_ref.concatenate(adata_test, batch_key="str_batch")

# make the batch category column
batch_id_labels = adata.obs["str_batch"].astype("category").cat.codes.values
adata.obs["batch_id"] = batch_id_labels # make batch_id to int8
celltype_id_labels = adata.obs["celltype"].astype("category").cat.codes.values # int8
...
) # np.array [cell_num, gene_num]
genes = adata.var["gene_name"].tolist()

celltypes_labels = adata.obs["celltype_id"].tolist()  # make sure count from 0
celltypes_labels = np.array(celltypes_labels) # np.array [cell_num, ]

batch_ids = adata.obs["batch_id"].tolist()
num_batch_types = len(set(batch_ids))
batch_ids = np.array(batch_ids)  # np.array [cell_num, ]
...
(
    ...
    all_counts, celltypes_labels, batch_ids, test_size=0.1, shuffle=True, random_state=42, stratify=celltypes_labels
)
...
# Process the reference data (train and valid)
...
# scGPT - INFO - train set number of samples: 19812, 
# 	 feature length: 22000
# scGPT - INFO - valid set number of samples: 2202, 
# 	 feature length: 22000
...
# %% inference
def test_raw(model: nn.Module, adata: DataLoader) -> float:
    all_counts = (
        adata.layers[input_layer_key].A
        if issparse(adata.layers[input_layer_key])
        else adata.layers[input_layer_key]
    ...
    return predictions, celltypes_labels, results, softmax_scores


# In[ ]:


predictions, labels, results, softmax_scores = test_raw(model, adata_test)
...
with open(save_dir / "labels.npy", "wb") as f:
    np.save(f, labels)
...
celltypes_new = adata.obs["celltype"].unique()
...
# plot UMAP using the original UMAP coordinates
...
with plt.rc_context({"figure.figsize": (10, 5), "figure.dpi": (300)}):
    ...
    plt.tight_layout()
    plt.savefig(save_dir / "results_with_novel_original.png", dpi=300)
...
for i in set([id2type[p] for p in predictions]):
    if i not in celltypes_new:
        celltypes_new.remove(i)
cm = confusion_matrix(labels+1, np.array(predictions)+1)
cm = cm.astype("float") / cm.sum(axis=1)[:, np.newaxis]
cm = pd.DataFrame(cm, index=cell_types_correct_order[:cm.shape[0]], columns=cell_types_correct_order[:cm.shape[1]])
plt.figure(figsize=(15, 15))
sns.heatmap(cm, annot=True, fmt=".1f", cmap="Blues")
plt.tight_layout() 
plt.savefig(save_dir / "confusion_matrix_with_novel.png", dpi=300)

results["test/confusion_matrix_with_novel"] = wandb.Image(
    str(save_dir / "confusion_matrix_with_novel.png"),
...
with plt.rc_context({"figure.figsize": (10,5), "figure.dpi": (300)}):
    ...
    plt.tight_layout()
    plt.savefig(save_dir / "results_with_novel_cell_embed_original.png", dpi=300)
...
with plt.rc_context({"figure.figsize": (10, 8), "figure.dpi": (300)}):
    ...
    plt.savefig(save_dir / "results_with_novel_cell_embed_original_v2.png", dpi=300)
...
adata_test_cell_emb.write(save_dir / "adata_test_cell_emb_mimetics-only.h5ad")
...
emb_df.to_csv(save_dir / 'ht6_embeddings_mimetics-only.csv', index=True)

adata_emb.obs.to_csv(save_dir / 'ht6_obs_info_mimetics-only.csv', index=True)

adata_emb.obs['predictions'].unique().tolist()
